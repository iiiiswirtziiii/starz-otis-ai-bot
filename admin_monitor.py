...
def summarize_spawn_row(server_name: str, detail: str, created_at: float) -> Optional[str]:
    ...
    #  3) [KITMANAGER] Successfully gave [elitekit2] to [CPTA1N]
    #
    # We only want to display ONE clean line:
    #   Server 10 elitekit2 1 11:08 p.m.
...
def summarize_spawn_line(
    server_name: str,
    detail: str,
    created_at: float,
) -> Optional[str]:
    """
    Turn a raw console 'detail' string into something short like:
      Server 10 c4 20 3:00 p.m.

    Returns None for noise lines we don't want to display (slot dumps, killfeed).
    """
    if not detail:
        return None
    ...
    # Ignore spammy slot dumps and killfeed lines
    if '<slot:"name">' in lt or "was killed by" in lt:
        return None

    # Try to parse "[ServerVar] giving CPTA1N 10 x Timed Explosive Charge"
    m = re.search(r"giving\s+\S+\s+(\d+)\s+x\s+(.+)", detail, re.IGNORECASE)
    # Friendly display mapping for some items
    item_aliases = {
        "timed explosive charge": "c4",
        "explosive.timed": "c4",
        "mlrs rocket": "mlrs rocket",
...
def summarize_spawn_row(server_name: str, detail: str, created_at: float) -> Optional[str]:
    """
    Convert a raw console spawn line into something like:
        Server 10 c4 20 3:00 p.m.

    Returns None for noise lines we don't want to show.
    """
    if not detail:
        return None
    ...
    # Try to match "[ServerVar] giving CPTA1N 10 x Timed Explosive Charge"
    m = re.search(r"giving\s+\S+\s+(\d+)\s+x\s+(.+)", detail, re.IGNORECASE)
    if m:
        amount = m.group(1)
        item_raw = m.group(2).strip().strip(".")
    ...
    # Fallback: keep a short snippet for non-standard spawns
    summary = detail.replace("\n", " ").strip()
    if len(summary) > 60:
        summary = summary[:57] + "..."
    return f"{server_name} {summary} {time_str}"

# admin_monitor.py  (add this helper)

from typing import Optional  # you probably already have this at the top
...
def fetch_admin_basic(admin_id: int) -> Optional[dict]:
    ...
    # Time string like "3:00 p.m."
    try:
        t = datetime.fromtimestamp(float(created_at))
    except Exception:
        t = datetime.utcnow()
    time_str = (
        t.strftime("%I:%M %p")
    ...
    if m:
        ...
        short_item = item_raw
        for key, val in item_aliases.items():
            if key in item_key:
                short_item = val
                break
        return f"{server_name} {short_item} {amount} {time_str}"

    # Fallback: shorten any other spawn detail
    summary = detail.replace("\n", " ").strip()
    if len(summary) > 60:
        summary = summary[:57] + "..."
    return f"{server_name} {summary} {time_str}"

from rcon_web import run_rcon_command, rcon_send_all
...
from typing import Optional  # you already import this at the top; if not, add it

def get_admin_profile(admin_id: int) -> Optional[dict]:
    """
    Look up an admin's Discord ID + main gamertag by admin_monitor ID.
    Returns {'discord_id': int, 'gamertag': str} or None.
    """
    conn = get_db_connection()
    cur = conn.cursor()
...
async def update_admin_log_for_admin(
    ...
    # ---- aggregate last-48h join/spawn counts ----
    ...
    lines.append("---------------------")
    if joins_by_server:
        for sname, j in joins_by_server.items():
            lines.append(f"- {sname}: `{j}` join(s)")
    else:
    ...
    max_desc_chars = 3500  # stay under Discord's 4096 limit
    ...
    # If we had a previous message recorded, delete it so we don't spam the channel
    if msg_obj:
        try:
