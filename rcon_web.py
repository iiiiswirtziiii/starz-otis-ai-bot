...
# MASTER RCON SWITCH
# =========================
# (no local RCON_ENABLED here – we use the one from config_starz)
# ======================================================
...
class WebRconClient:
    ...
    async def connect(self):
        """
        Open (or reopen) a WebRCON connection.
        If we already have a connection object, we reuse it.
        """
    ...
    async def close(self):
        """
        Close the WebRCON connection if it exists.
        """
        if self.ws is not None:
            try:
                await self.ws.close()
    ...
    async def _recv_until_id(self, identifier: int, timeout: float = 5.0) -> dict:
        """
        Read messages until we see one with the matching Identifier.
        """
        ...
        while True:
            ...
            # Only return when the Identifier matches this command
            if data.get("Identifier") == identifier:
                return data

            # Otherwise, it's just regular console spam; ignore it.
            # If you want, you can log it here.
...
async def check_rcon_health_on_startup() -> list[str]:
    ...
    for key, client in rcon_manager.clients.items():
        try:
            # Only connect if we don't already have a live socket
...
async def run_rcon_command(command: str, client_key: str = "s1") -> Optional[dict]:
    ...
    - Returns the RCON response dict on success, or None on failure.
    """
    if not RCON_ENABLED:
    ...
    try:
        # send_command already calls connect() internally and waits for reply
        resp = await client.send_command(command)
        print(f"[RCON] Sent → [{client_key.upper()}] {command}")
        # Optional: tiny debug of the response
