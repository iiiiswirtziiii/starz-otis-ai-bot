...
from nuke_rewards import maybe_handle_nuke_purchase
...
RCON_DISCONNECT_COOLDOWN = {}  # server_key -> timestamp
...
async def handle_shop_log_message(message: discord.Message) -> None:
    ...
    # ... your unban + thank-you logic above ...
...
async def handle_rcon_console_line(server_key: str, msg_text: str, created_at_ts: float) -> None:
    """
    Called for each console line from a given server's WebRCON connection.
    This is basically the inner logic of your old rcon_console_watch loop.
    """
    from admin_promotion_watch import maybe_handle_admin_promotion
    from admin_mon_system import log_admin_activity_for_ids
    from promoter_mon_system import maybe_handle_promoter_spawn
    ...
    # We'll just treat all RCON hits here as "spawn" events tied to this server.
    server_name = server_key  # or "Server 10" etc.

    # 3) Admin monitor log update (new API from admin_mon_system)
    ...
    # Also treat high-risk kits as triggers if they appear in kitmanager logs
    if matched_item is None and "[kitmanager]" in lt:
        for kit in HIGH_RISK_KITS:
            if kit.lower() in lt:
                matched_item = kit
                break

    # Only enforce if we actually matched a high-risk item/kit
    if matched_item is not None:
        await handle_spawn_enforcement_for_event(
            admin_id=matching_admin_ids[0],
            ...
            async with websockets.connect(url, ping_interval=None) as ws:
                ...
                async for raw in ws:
                    ...
                    try:
                        ...
                        # 5) High-risk spawn enforcement (rockets/C4/MLRS + kits)
                        lt = msg_text.lower()
                        matched_item: Optional[str] = None

                        # Item-based (rockets/C4/MLRS) hits
                        for item in HIGH_RISK_SPAWN_ITEMS:
                            if item.lower() in lt:
                                matched_item = item
                                break
...
async def on_ready():
    ...
    if not RCON_ENABLED:
        ...
        try:
            # This will connect each WebRconClient once and keep the sockets open
            failures = await check_rcon_health_on_startup()
            rcon_failures.extend(failures)
        except Exception as e:
    ...
    # Startup embed
    try:
        # Send the OTIS is BACK ONLINE embed here:
...
async def register_staff_slash(
    ...
    # ===== PROMOTER PATH – store in the same admin DB, but treated as promoter via roles =====
    if staff_type is StaffType.PROMOTER:
        # 1) Store/update in admin_monitor_admins just like an admin
        admin_id = register_or_update_admin(
            discord_user=member,
            main_gamertag=gamertag,
            alt_gamertag=alt_gamertag,
        ...
        # 2) Optionally build/update their admin log embed (so they get a log too)
        try:
            await update_admin_log_for_admin(
                bot,
                admin_id,
            ...
            print(f"[ADMIN-MONITOR] Updated admin log for promoter admin_id={admin_id}")
        except Exception as e:
            print(f"[ADMIN-MONITOR] Failed to update promoter log after register: {e}")

        # 3) Confirmation back to the command user
        await interaction.followup.send(
            embed=discord.Embed(
...
async def nukecheck(interaction: discord.Interaction):
    ...
    for idx, entry in enumerate(stats, start=1):
        ...
        lines.append(
            f"**{idx}.** {buyer} – **{claims}** claim(s) – {when}"
        )
...
async def on_message(message: discord.Message):
    ...
    #     channel.id in PLAYER_FEED_CHANNEL_IDS or channel.id in ADMIN_FEED_CHANNEL_IDS
    # ):
    ...
    if is_staff:
        # Stop AI replies for this ticket from now on
        active_ai_channels.discard(channel.id)

        # Also mark the in-memory session as disabled if it exists
        session = ticket_sessions.get(channel.id)
        if session is not None:
            session["ai_disabled"] = True

        # From this point on, no workflows / kit helpers / AI should answer automatically
        return
